...
public interface StudentsTaskRepository extends JpaRepository<StudentsTask, Long>, JpaSpecificationExecutor<StudentsTask> {
    @Query("select distinct s.task, s.id from StudentsTask s where lower(s.task.name) like lower(CONCAT('%', :taskName, '%')) and s.student.id = :studentId")
    Page<Task> findAllTaskByTaskNameLikeAndStudentId(String taskName, Long studentId, Pageable pageable);
    @Query("select distinct s.task.course, s.id from StudentsTask s where lower(s.task.course.name) like lower(CONCAT('%', :courseName, '%')) and s.student.id = :studentId")
